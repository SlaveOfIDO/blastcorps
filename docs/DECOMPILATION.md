# Random notes collected from others and me

## general

- ternaries don't always match if/else statements; sometimes even writing the 'else' on a new line affects codegen

## 64 bit stuff

- if (0 != 0) indicates bigint
- if a bnez or bez is at the wrong place but other instructions fit and there are two variables involved that are s32/u32 and those variables are nearby, then it is probably a 64 bit integer
- if a 64 bit int is used but only the lower bits are used, then it is a cast to s32
- when it comes to 64-bit writes the compiler should know if its declared in the same file as the code then it can know that it can optimize the write
- `0x10000000000 ^ X == 0` can be rewritten to `X == 0x10000000000`

## branches

- if a useless branch with s0 register is used, then its likely `if(cond?1:0)`
- if + var_sx is ternary
-
## floating points

- if some registers are swapped when comparing, then you need to swap the comparision
- extranous `mtc1 zero` with rodata literal? then it is a ternary
...
- how to get s0?
  ...
  Option 4: Use +=, -=, etc.
- v0 is copied into s0 needlessly? then it is assignment in first parameter of following function
- how to get a0,a1,a2 in a condition?
  Option 1: Use switch (even though it's the only case)
- cannot get return value directly into v0? try changing return type from s32 to int
- swapped regs with addu? Try to fold the following line of code into the previous one
...
- two instructions swapped around? -> use , instead of ; or put statements on the same line

## instruction mismatch

- move instead of li? -> perhaps an u64 that is currently split to 2 u32
- extra `nop`s -> most of the times this is due to alignment. You can ignore this
- Do you need lw instead of addiu?
...
- if the stack size is still wrong even if all temporaries are deleted, then 64 bit arithmetic could potentially create shadow 64 bit stack variables. Try to fold 64 bit arithmetic together
  instead of
  sp38 = (D_803649D8 / 0x64);
  D_803649E2 = (sp38 % 0x28) - 0x14;
  it is
  D_803649E2 = ((D_803649D8 / 0x64) % 0x28) - 0x14;
...
- unsigned vs signed can cause registers to move
- abstract struct copies (e.g. 0x28E tomfoolery in Fighter_Struct bitfield) will get a stack address first and then store
- size matters (bitfield declarations: u8 : 1 vs u16 : 1 vs u32 : 1 may affect codegen)
- occasionally, array index comparisons get optimized so that IDO compares pointers (see https://decomp.me/scratch/Hxefw)
- watch out for hidden & 0xFFFF operations (thanks HAL)
- bitfield comparisons: if the comparison of a variable generates *sll*, *srl* or *sra* in a statement, then it is a bitfield; if it is always
  ANDed with *andi*, it is not a bitfield
- be careful with chained bitfield assignments (var1 = var2 = var3 = FALSE, this one is annoying but you can tell this is happening because IDO chains the previous variable's bitwise operation into the next)
- else return 0 VS simple return 0
- floats without decimal points VS with decimal points (e.g. x = 0.0F VS x = 0)
- if(my_u8_or_u16_var) VS ((u32)my_u8_or_u16_var)
...
`-g` and `-O2` place things on the stack in the same way.
(TODO: verify this.) However, the optimizer can then move some variables into registers,
leaving slots empty. If an explicitly declared stack variable is unused or placed in a register,
and it is last in the declaration order, it won't affect `sp`.
