                     ...
                     Acmd *p)
{
    ...
    while (e->ctrlList != 0) {
        ...
        #line 104
        AUDIO_ASSERT(samples >= 0);
        AUDIO_ASSERT(samples <= AL_MAX_RSP_SAMPLES);
        ...
        switch (e->ctrlList->type) {
	      ...
	      ptr = _pullSubFrame(e, &inp, &loutp, samples, sampleOffset, ptr);
              ...
              if (e->delta >= e->segEnd){
                   ...
                   */
                //e->cvolL += (s16) (s32) (f32) ((f64) (f32) e->cvolL + ((f64) ((f32) ((f64) ((f32) (e->lratm << 16) + e->lratl) / 65536.0) * (f32) e->delta) * 0.125));
                //e->cvolR += (s16) (s32) (f32) ((f64) (f32) e->cvolR + ((f64) ((f32) ((f64) ((f32) (e->rratm << 16) + e->rratl) / 65536.0) * (f32) e->delta) * 0.125));
                    ...
                    s32 sampleOffset, Acmd *p)
{
  ...
  if (!outCount)
	...
	e->first = 0;
         ...
         */
        e->ltgt = (e->volume * eqpower[e->pan]) >> 15;
        e->lratm = _getRate(e->cvolL, e->ltgt,
                            e->segEnd, &(e->lratl));
        e->rtgt = (e->volume *
                   eqpower[EQPOWER_LENGTH - e->pan - 1]) >> 15;
        e->rratm = _getRate(e->cvolR, e->rtgt, e->segEnd,
                            &(e->rratl));
