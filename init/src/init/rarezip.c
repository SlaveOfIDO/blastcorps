...
s32 func_init_8022043C(void) {
    ...
    D_init_80222A1C += 2; // Skip gzip magic number 0x1F 0x8B
    D_80222840 = -1;
    D_init_802229FC = 0;
    D_80222840 = D_init_802229F0[D_init_80222A1C++];
    ...
    flags = D_init_802229F0[D_init_80222A1C++];
    D_init_80222A1C += 6; // Skip rest of header
    
    if (flags & 2) { // has crc-16 checksum?
        D_init_80222A1C += 2; // then skip crc-16 checksum
    }

    if (flags & 4) { // has additional information?
        // read length of additional information
        sp28 = (D_init_802229F0[D_init_80222A1C++]);
        sp28 |= (D_init_802229F0[D_init_80222A1C++] << 8);
        
        // skip additional information
        D_init_80222A1C += sp28;
    }
  
    if (flags & 8) { // has original filename?
        while(D_init_802229F0[D_init_80222A1C++] != 0); // skip filename
    }

    if (flags & 16) { // has additional comment?
        while(D_init_802229F0[D_init_80222A1C++] != 0); // skip comment
    }
    // Skip crc32 checksum (4 byte) and size of original file (4 byte)
    D_init_802229FC = D_init_80222A1C + 8;
    return D_80222840;
}
